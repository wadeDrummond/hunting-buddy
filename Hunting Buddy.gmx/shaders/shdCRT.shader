...
uniform bool border;
    ...
    float inputGamma = 2.0;
    float outputGamma = 2.8;
    
    vec4 scanlineWeights(float dist, vec4 col)
    {
        vec4 wid = 0.5 + 2.0 * pow(col, vec4(2.0));
        vec4 weights = vec4(dist);                
        return 0.51 * exp(-pow(abs(weights * sqrt(8.0 / wid)), wid)) / (0.18 + 0.06 * wid);
        //return 1 * exp(-pow(abs(weights * sqrt(1) / (1);
    }
...
void main()
{
    ...
    vec4 weights2 = scanlineWeights(1.0, col2);
